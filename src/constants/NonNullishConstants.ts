...
export const NonNullishConstantsSchema = z.object({
  NON_NULLISH_DATETIMEOFFSET: z.literal("2900-01-01T00:00:00.000Z"),
...
export const {
  ...
  NON_NULLISH_DATETIMEOFFSET:
    NonNullishConstantsSchema.shape.NON_NULLISH_DATETIMEOFFSET.value,
  NON_NULLISH_DECIMAL:
...
export const NON_NULLISH_DATETIMEOFFSET_SCHEMA = z.date().refine(
  (date) => {
    return date.toISOString() === NON_NULLISH_DATETIMEOFFSET;
  },
  { error: `Invalid input: expected ${NON_NULLISH_DATETIMEOFFSET}` },
);
